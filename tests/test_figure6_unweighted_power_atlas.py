...
from src.scripts import figure6_unweighted_power_atlas as figure6


class Figure6UnweightedPowerAtlasTest(unittest.TestCase):
    def test_default_bins_include_ten_thousand_calibration_center(self):
        centers = (figure6.N_EFF_BINS[:-1] + figure6.N_EFF_BINS[1:]) / 2.0

        self.assertTrue(np.isclose(centers[-1], 4.0))

    def test_ordinary_conformal_p_values_use_upper_tail_with_self_atom(self):
        p_values = figure6.ordinary_conformal_p_values(
    ...
    def test_tiny_run_writes_unweighted_power_atlas_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path_patches = {
                "OUT_DIR": root,
                "POWER_ATLAS_FIGURE_PATH": root / "figure6_unweighted_power_atlas.png",
            ...
            with mock.patch.multiple(figure6, **path_patches), mock.patch.multiple(
                figure6,
                **value_patches,
            ):
                figure6.main(
            ...
            self.assertEqual(set(atlas["score_regime"]), set(figure6.SCORE_REGIMES))
            ...
            self.assertEqual(set(atlas_tikz["figure"]), {"figure6"})
            ...
            self.assertEqual(set(atlas_reference["figure"]), {"figure6"})
            ...
            self.assertEqual(set(fdr_tikz["figure"]), {"figure6_supp"})
            ...
            self.assertEqual(set(fdr_reference["figure"]), {"figure6_supp"})
