...
from wkde_cad.empirical.benchmark import model_selection
from wkde_cad.empirical.benchmark.utils.splits import (
...
class SharedModelSelectionTest(unittest.TestCase):
    def test_empirical_result_cache_requires_the_exact_design_grid(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "demo.csv"
            pd.DataFrame(
                [
                    {"seed": 1, "severity": 0.0, "approach": "empirical"},
                    {
                        "seed": 1,
                        "severity": 0.0,
            ...
            self.assertTrue(
                experiment._results_are_current(
                    path,
                    seeds=[1],
                    severities=[0.0, 1.0],
                    approaches=["empirical", "empirical_weighted"],
                )
            )
            self.assertFalse(
                experiment._results_are_current(
    ...
    def test_existing_selection_csv_skips_without_loading_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch(
                "wkde_cad.empirical.benchmark.model_selection.load",
    ...
    def test_incomplete_selection_csv_reruns_when_configured_models_are_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch(
                    "wkde_cad.empirical.benchmark.model_selection.get_dataset_enum",
                    return_value="demo",
                ),
                mock.patch(
                    "wkde_cad.empirical.benchmark.model_selection.load",
                ...
                mock.patch(
                    "wkde_cad.empirical.benchmark.model_selection.process_seed_phase1",
    ...
    def test_model_selection_uses_only_model_selection_pools(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with (
                ...
                mock.patch(
                    "wkde_cad.empirical.benchmark.model_selection.get_model_instance",
    ...
    def test_covariate_shift_config_snapshot_tracks_the_active_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
