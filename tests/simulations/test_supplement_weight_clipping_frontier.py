...
from wkde_cad.simulations import supplement_weight_clipping_frontier as clipping


class WeightClippingFrontierTest(unittest.TestCase):
    def test_default_output_dir_is_under_outputs(self):
        self.assertEqual(clipping.OUT_DIR.name, "supplement_weight_clipping_frontier")
        self.assertEqual(clipping.OUT_DIR.parent.name, "simulations")
        self.assertEqual(clipping.OUT_DIR.parent.parent.name, "outputs")

    def test_unclipped_analytic_tail_equals_shifted_null_tail(self):
        rho = 1.2
        scores = np.array([-1.0, 0.0, 1.0, 2.5, 4.0])

        observed = clipping.clipped_target_tail(scores, cap=np.inf, rho=rho)
    ...
    def test_tighter_clipping_increases_oracle_tail_mismatch(self):
        ...
        unclipped = clipping.oracle_tail_metrics(
            cap=np.inf,
            tail_probs=tail_probs,
            rho=1.5,
        )
        clipped = clipping.oracle_tail_metrics(
            cap=1.0,
            tail_probs=tail_probs,
            rho=1.5,
        )
    ...
    def test_tighter_clipping_lowers_self_atom_and_raises_ess(self):
        ...
        tight = clipping.resolution_metrics(calib_z, test_z, cap=1.0, rho=1.5)
        loose = clipping.resolution_metrics(calib_z, test_z, cap=np.inf, rho=1.5)
    ...
    def test_unclipped_tv_is_zero_and_tight_clipping_has_larger_tv(self):
        self.assertAlmostEqual(
            clipping.clipped_target_tv(cap=np.inf, rho=1.5),
            0.0,
        )

        tight = clipping.clipped_target_tv(cap=1.0, rho=1.5)
        loose = clipping.clipped_target_tv(cap=89.0, rho=1.5)
    ...
    def test_tiny_run_writes_outputs_and_valid_multi_rho_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path_patches = {
                "OUT_DIR": root,
                "FIGURE_PATH": root / "weight_clipping_frontier.png",
                "SUMMARY_PATH": root / "weight_clipping_frontier_summary.csv",
                "TIKZ_PATH": root / "weight_clipping_frontier_tikz.csv",
                "TABLE_PATH": root / "weight_clipping_frontier_key_settings.csv",
            }
            value_patches = {
                "RHO_VALUES": [0.5, 1.5],
                "N_CAL": 40,
            ...
            with mock.patch.multiple(clipping, **path_patches), mock.patch.multiple(
                clipping,
                **value_patches,
            ):
                clipping.main()
            ...
            self.assertEqual(set(summary["summary_version"]), {clipping.SUMMARY_VERSION})
            ...
            self.assertEqual(set(tikz["figure"]), {"weight_clipping_frontier"})
