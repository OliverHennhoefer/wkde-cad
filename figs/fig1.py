...
ax1.hist(calibration_scores, bins=50, alpha=0.5, color=color_calibration,
         label='Calibration', density=True, edgecolor='none')

# Plot stacked test histograms (normal + anomaly)
ax1.hist([estimates_raw_normal, estimates_raw_anomaly], bins=50,
         alpha=0.5, color=[color_test, color_anomaly],
         label=['Test (Normal)', 'Test (Anomaly)'],
         density=True, edgecolor='none', stacked=True)
...
ax2.hist(estimates, bins=50, alpha=0.7, color=color_test,
         edgecolor='none')
...
if np.any(mask_anomaly):
    ax3.scatter(ranks[:zoom_limit][mask_anomaly],
               sorted_pvals[:zoom_limit][mask_anomaly],
               c=color_anomaly, s=10, alpha=0.6, edgecolor='none',
               label='Anomaly', zorder=3)
if np.any(mask_normal):
    ax3.scatter(ranks[:zoom_limit][mask_normal],
               sorted_pvals[:zoom_limit][mask_normal],
               c=color_test, s=10, alpha=0.6, edgecolor='none',
               label='Normal', zorder=2)

# Plot BH threshold line
ax3.plot(ranks[:zoom_limit], bh_threshold[:zoom_limit],
        'k--', linewidth=1.5, alpha=0.8, label=f'BH threshold (α={alpha})',
        zorder=1)
