...
OUT_ROOT = REPO_ROOT / "outputs" / "figure1"
...
def build_heatmap_summary(mode: str, pruning: str) -> pd.DataFrame:
    ...
    with ProcessPoolExecutor(max_workers=WORKERS) as executor:
        ...
        for task, counts_by_kappa in tqdm(
            futures,
            total=total_tasks,
            desc=f"Figure 1 {mode} heatmap",
            unit="cell",
        ):
...
def build_collapse_summary(mode: str, pruning: str) -> pd.DataFrame:
    ...
    with ProcessPoolExecutor(max_workers=WORKERS) as executor:
        ...
        for collapse_update in tqdm(
            futures,
            total=total_tasks,
            desc=f"Figure 1 {mode} collapse",
            unit="block",
        ):
...
def main(argv: list[str] | None = None) -> None:
    ...
    for mode in tqdm(args.modes, desc="Figure 1 modes", unit="mode"):
