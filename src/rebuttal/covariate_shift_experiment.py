...
def _make_weight_estimator(
    ...
    if weight_mode == "estimated":
        return _build_estimated_weight_estimator(
            ...
            cfg["global"]["n_bootstraps"],
        )
    raise ValueError("rebuttal_covariate_shift.weight_mode must be 'oracle' or 'estimated'.")
...
def process_shift_seed(
    ...
    train_split = cfg["global"]["train_split"]
    test_use_proportion = cfg["global"]["test_use_proportion"]
    test_anomaly_rate = cfg["global"]["test_anomaly_rate"]
    weight_mode = str(rebuttal_cfg.get("weight_mode", "oracle")).strip().lower()
    ...
    for approach_name, approach_config in approaches.items():
        ...
        if approach_config["weighted"]:
            decisions = weighted_false_discovery_control(
                ...
                alpha=cfg["global"]["fdr_rate"],
                pruning=pruning_method,
                seed=seed,
            )
        else:
            decisions = false_discovery_control(p_values, method="bh") <= cfg["global"][
                "fdr_rate"
            ]
...
def run_experiment(
    *,
    ...
    logger = get_logger("rebuttal_covariate_shift")
    output_dir.mkdir(parents=True, exist_ok=True)
    model_selection_dir = REPO_ROOT / "outputs" / "model_selection"
    pruning_method = _pruning_method(cfg["global"].get("pruning", "heterogeneous"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run standalone rebuttal covariate-shift experiments."
    )
...
def main() -> None:
    ...
    rebuttal_cfg = cfg.get("rebuttal_covariate_shift")
    if rebuttal_cfg is None:
        raise ValueError("Missing [rebuttal_covariate_shift] section in config.")

    datasets = args.datasets or _as_list(cfg["experiments"]["datasets"])
    seeds = args.seeds or _seed_list(cfg["global"]["meta_seeds"])
    severities = args.severities or [float(v) for v in _as_list(rebuttal_cfg["severities"])]
    approaches_to_run = args.approaches or _as_list(
        rebuttal_cfg.get("approaches", cfg["global"]["approaches"])
    )
    output_dir = args.output_dir or (REPO_ROOT / rebuttal_cfg["output_dir"])
