...
OUT_DIR = REPO_ROOT / "outputs" / "simulations" / "supplement_weight_clipping_frontier"
FIGURE_PATH = OUT_DIR / "weight_clipping_frontier.png"
SUMMARY_PATH = OUT_DIR / "weight_clipping_frontier_summary.csv"
TIKZ_PATH = OUT_DIR / "weight_clipping_frontier_tikz.csv"
TABLE_PATH = OUT_DIR / "weight_clipping_frontier_key_settings.csv"
...
def build_summary() -> pd.DataFrame:
    ...
    trial_rows = []
    tasks = [(rho, seed) for rho in rhos for seed in range(int(N_SEEDS))]
    for rho, seed in tqdm(
        tasks,
        total=len(tasks),
        desc="weight-clipping trials",
        unit="seed",
    ):
        trial_rows.extend(simulate_seed(seed, rho, caps))
...
def load_or_build_summary() -> pd.DataFrame:
    if summary_is_current():
        print("loading existing weight-clipping summary", flush=True)
...
def validate_summary(summary: pd.DataFrame) -> None:
    ...
    if summary.duplicated(["rho", "clip_cap_label"]).any():
        raise RuntimeError("Weight-clipping summary contains duplicate rho/cap pairs.")
    finite_columns = [
        "plot_x",
            ...
            rows.append(
                {
                    ...
                    "figure": "weight_clipping_frontier",
                    "panel": panel,
                    "panel_title": group,
