...
PLOT_PATH = OUT_DIR / "randomized_pvalue_instability.png"
SUMMARY_PATH = OUT_DIR / "randomized_pvalue_instability_summary.csv"
DISTRIBUTION_PATH = OUT_DIR / "randomized_pvalue_instability_distribution.csv"
INTERVAL_TIKZ_PATH = OUT_DIR / "randomized_pvalue_instability_intervals_tikz.csv"
DISTRIBUTION_TIKZ_PATH = (
    OUT_DIR / "randomized_pvalue_instability_distribution_tikz.csv"
)
...
def sorted_calibration(
    ...
    return sorted_scores, suffix_weights
...
def build_fixed_world(rho: float, world_seed: int) -> FixedWorld:
    ...
    sorted_scores, suffix_weights = sorted_calibration(calib_scores, calib_weights)
...
def build_results(
    ...
    for world in tqdm(
        worlds,
        total=total_worlds,
        desc="Randomized p-value fixed worlds",
        unit="world",
    ):
...
def load_or_build_results(
    ...
    if summaries_are_current():
        print("loading existing randomized p-value summaries", flush=True)
...
def validate_results(summary: pd.DataFrame, distribution: pd.DataFrame) -> None:
    ...
    if observed_pairs != expected_pairs:
        raise RuntimeError(
            "Unexpected randomized p-value fixed worlds: "
    ...
    if summary.duplicated(["rho", "world_seed"]).any():
        raise RuntimeError("Summary contains duplicate fixed worlds.")
    if not bool(summary["inliers_nonrejectable"].all()):
        raise RuntimeError("Inlier p-value intervals must stay above alpha.")
    probability_columns = [
...
def plot_randomized_pvalue_instability(
    worlds: list[FixedWorld],
    summary: pd.DataFrame,
) -> None:
            ...
            rows.append(
                {
                    ...
                    "figure": "randomized_pvalue_instability",
                    "panel": "A",
...
def main() -> None:
    ...
    plot_randomized_pvalue_instability(worlds, summary)
