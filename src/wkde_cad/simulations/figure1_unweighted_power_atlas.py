...
OUT_DIR = REPO_ROOT / "outputs" / "simulations" / "figure1_unweighted_power_atlas"
POWER_ATLAS_FIGURE_PATH = OUT_DIR / "figure1_unweighted_power_atlas.png"
POWER_ATLAS_SUMMARY_PATH = OUT_DIR / "figure1_unweighted_power_atlas_summary.csv"
POWER_ATLAS_TIKZ_PATH = OUT_DIR / "figure1_unweighted_power_atlas_tikz.csv"
POWER_ATLAS_REFERENCE_TIKZ_PATH = (
    OUT_DIR / "figure1_unweighted_power_atlas_reference_tikz.csv"
)
FDR_RATIO_ATLAS_FIGURE_PATH = OUT_DIR / "figure1_supp_fdr_ratio_atlas.png"
FDR_RATIO_ATLAS_TIKZ_PATH = OUT_DIR / "figure1_supp_fdr_ratio_atlas_tikz.csv"
FDR_RATIO_ATLAS_REFERENCE_TIKZ_PATH = (
    OUT_DIR / "figure1_supp_fdr_ratio_atlas_reference_tikz.csv"
)
...
def run_cells(tasks: list[CellTask], workers: int) -> pd.DataFrame:
    ...
    if workers <= 1:
        ...
        for cell_rows in tqdm(iterator, total=len(tasks), desc="Figure 1 cells", unit="cell"):
...
def load_or_build_power_atlas_summary(
    *,
    ...
    if not force and summary_is_current(
        ...
        print("loading existing Figure 1 power-atlas summary", flush=True)
...
def validate_power_atlas_summary(summary: pd.DataFrame) -> None:
    if summary.empty:
        raise RuntimeError("Figure 1 power summary is empty.")
    if set(summary["collection"]) != {"atlas"}:
        raise RuntimeError("Figure 1 summary contains an unexpected collection.")
    if (summary["count"] <= 0).any():
        raise RuntimeError("Figure 1 summary contains zero-count cells.")
    repeat_columns = {"calibration_repeats", "test_repeats_per_calibration"}
    if not repeat_columns.issubset(summary.columns):
        raise RuntimeError("Figure 1 summary must record nested Monte Carlo repeats.")
    expected_count = (
    ...
    if not (summary["count"].astype(int) == expected_count).all():
        raise RuntimeError("Figure 1 summary count does not match nested repeats.")
    probability_columns = [
    ...
    if not np.allclose(summary["mean_neff"], summary["mean_n_cal"]):
        raise RuntimeError("Unweighted Figure 1 must have N_eff equal to N_cal.")
    if not np.allclose(summary["mean_rho"], 0.0):
        raise RuntimeError("Unweighted Figure 1 must not include covariate shift.")
    ...
    if observed != expected:
        raise RuntimeError(
            f"Figure 1 atlas cells incomplete: missing={len(expected - observed)}, "
...
def validate_outputs() -> None:
    for path in [
        ...
        if not path.exists() or path.stat().st_size == 0:
            raise RuntimeError(f"Expected Figure 1 output was not written: {path}")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Figure 1 standard conformal CAD power atlas.",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def main(argv: list[str] | None = None) -> None:
    ...
    validate_outputs()
    print("Figure 1 outputs written:", flush=True)
