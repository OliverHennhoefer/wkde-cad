...
def _process_seed_phase1_task(task):
    return process_seed_phase1(*task)
...
def run_model_selection(
    *,
    ...
    for ds_name in datasets:
        ...
        if jobs == 1:
            results = [
                _process_seed_phase1_task(task)
                for task in tqdm(
                    tasks,
                    total=len(tasks),
                    desc=f"{ds_name} model selection",
                    unit="seed",
                )
            ]
        else:
            with mp.Pool(jobs) as pool:
                results = list(
                    tqdm(
                        pool.imap(_process_seed_phase1_task, tasks),
                        total=len(tasks),
                        desc=f"{ds_name} model selection",
                        unit="seed",
                    )
