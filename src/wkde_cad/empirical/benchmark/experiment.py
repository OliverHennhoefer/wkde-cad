...
from .utils.registry import get_dataset_enum, get_model_instance
from .utils.splits import split_model_selection_evaluation
from .utils.weight_estimators import build_weight_estimator
...
DEFAULT_CONFIG = Path(__file__).with_name("empirical_benchmark.toml")
...
def _make_weight_estimator(
    ...
    if weight_mode == "estimated":
        return _build_estimated_weight_estimator(
            ...
            int(cfg["weighting"].get("n_bootstraps", 50)),
        )
...
def _build_approaches(
    *,
    ...
    split_calib = cfg["conformal"].get("split_calib", 0.5)
    return {
        "empirical": {
            "strategy": Split(n_calib=split_calib),
...
def process_shift_seed(
    ...
    for approach_name, approach_config in approaches.items():
        try:
            ...
            get_logger("empirical_benchmark").warning(
...
def _process_shift_seed_task(task):
    return process_shift_seed(*task)
...
def _results_are_current(
    ...
    *,
    ...
    """Return whether a dataset CSV exactly covers the requested design grid."""

    if not path.exists():
        return False
    try:
        results = pd.read_csv(path, usecols=["seed", "severity", "approach"])
    except (OSError, ValueError, pd.errors.ParserError):
        return False
    expected = {
        (int(seed), round(float(severity), 12), approach)
        for seed in seeds
        for severity in severities
        for approach in _approaches_for_severity(approaches, float(severity))
    }
    observed = {
        (int(row.seed), round(float(row.severity), 12), str(row.approach))
...
def _copy_config_snapshot(
    ...
    if (
        ...
        and not force
        and snapshot_path.read_bytes() == config_path.read_bytes()
    ):
...
def run_experiment(
    *,
    ...
    logger = get_logger("empirical_benchmark")
    ...
    for ds_name in tqdm(datasets, desc="Empirical benchmark datasets", unit="dataset"):
        results_csv = output_dir / f"{ds_name}.csv"
        if not force and _results_are_current(
            results_csv,
            seeds=seeds,
        ...
        if jobs == 1:
            raw_results = [
                _process_shift_seed_task(task)
                for task in tqdm(
                    tasks,
                    total=len(tasks),
                    desc=f"{ds_name} shifted tasks",
                    unit="task",
                )
            ]
        else:
            with mp.Pool(jobs) as pool:
                raw_results = list(
                    tqdm(
                        pool.imap(_process_shift_seed_task, tasks),
                        total=len(tasks),
                        desc=f"{ds_name} shifted tasks",
                        unit="task",
                    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run empirical covariate-shift conformal anomaly detection benchmarks."
    )
